...
class CameraDebugPage(QWidget):
    ...
    def init_ui(self):
        ...
        self.port_combo.setStyleSheet("font-size: 9pt; color: #333333;")
        self.refresh_ports_btn = QPushButton("🔄")
        self.refresh_ports_btn.setFixedSize(32, 32)
        self.refresh_ports_btn.setStyleSheet(get_page_button_style('camera', 'refresh', width=32))
        ...
        self.baudrate_combo.addItems(["1000000", "1500000", "2000000", "3000000"])
        baudrate_label = QLabel("波特率")
        baudrate_label.setStyleSheet("font-size: 9pt; background-color: transparent;")
        serial_config_layout.addRow(baudrate_label, self.baudrate_combo)

        # 数据位
        self.databits_combo = QComboBox()
        self.databits_combo.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        ...
        serial_config_layout.addRow(databits_label, self.databits_combo)

        # 校验位
        self.parity_combo = QComboBox()
        self.parity_combo.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        ...
        serial_config_layout.addRow(parity_label, self.parity_combo)

        # 停止位
        self.stopbits_combo = QComboBox()
        self.stopbits_combo.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        ...
        serial_config_layout.addRow(stopbits_label, self.stopbits_combo)

        # 连接/断开按钮
        self.connect_btn = QPushButton("📷连接Camera串口")
        ...
        serial_config_layout.addRow(self.connect_btn)
        ...
        self.image_format_combo.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        self.image_format_combo.setStyleSheet("font-size: 9pt; color: #333333;")
        self.image_format_combo.addItems(["YUV422", "YUV420", "RGB565", "RGB888", "JPEG", "MJPEG"])
        self.image_format_combo.currentTextChanged.connect(self.update_image_format)
        image_format_label = QLabel("图像格式")
        image_format_label.setStyleSheet("font-size: 9pt; background-color: transparent;")
        image_format_layout.addRow(image_format_label, self.image_format_combo)

        # 图像类型（灰度/彩色）
        self.image_type_combo = QComboBox()
        self.image_type_combo.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        self.image_type_combo.setStyleSheet("font-size: 9pt; color: #333333;")  # 修改字体颜色为深灰色
        self.image_type_combo.addItems(["彩色图像", "灰度图像"])
        self.image_type_combo.setCurrentText("彩色图像")
        self.image_type_combo.currentTextChanged.connect(self.update_image_type)
        image_type_label = QLabel("图像类型")
        image_type_label.setStyleSheet("font-size: 9pt; background-color: transparent;")
        image_format_layout.addRow(image_type_label, self.image_type_combo)

        # 图像尺寸选择框
        self.image_size_combo = QComboBox()
        self.image_size_combo.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        self.image_size_combo.setStyleSheet("font-size: 9pt; color: #333333;")
        self.image_size_combo.addItems(["VGA (640x480)", "QVGA (320x240)"])
        self.image_size_combo.currentIndexChanged.connect(self.update_image_size)
        image_size_label = QLabel("图像尺寸")
        image_size_label.setStyleSheet("font-size: 9pt; background-color: transparent;")
        image_format_layout.addRow(image_size_label, self.image_size_combo)

        # 字节顺序
        self.byte_order_combo = QComboBox()
        self.byte_order_combo.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        self.byte_order_combo.setStyleSheet("font-size: 9pt; color: #333333;")
        self.byte_order_combo.addItems(["Big Endian", "Little Endian"])
        byte_order_label = QLabel("字节顺序")
        byte_order_label.setStyleSheet("font-size: 9pt; background-color: transparent;")
        image_format_layout.addRow(byte_order_label, self.byte_order_combo)

        # YUV格式
        self.yuv_format_combo = QComboBox()
        self.yuv_format_combo.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        self.yuv_format_combo.setStyleSheet("font-size: 9pt; color: #333333;")
        self.yuv_format_combo.addItems(["YUYV", "UYVY", "VYUY", "YVYU"])
        yuv_format_label = QLabel("YUV格式")
        yuv_format_label.setStyleSheet("font-size: 9pt; background-color: transparent;")
        image_format_layout.addRow(yuv_format_label, self.yuv_format_combo)

        # 帧同步
        self.frame_sync_check = QCheckBox("启用帧同步")
        self.frame_sync_check.setChecked(False)
        self.frame_sync_check.setStyleSheet("font-size: 9pt; color: #333333;")
        self.frame_sync_check.toggled.connect(self.toggle_frame_sync)
        image_format_layout.addRow(self.frame_sync_check)

        # 应用配置按钮
        """
        apply_btn = QPushButton("应用配置")
        apply_btn.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        apply_btn.setStyleSheet(get_page_button_style('camera', 'apply_format'))
        apply_btn.clicked.connect(self.apply_image_format)
        image_format_layout.addRow(apply_btn)
        """

        config_layout.addWidget(image_format_group)
        ...
        self.image_size_label.setStyleSheet("font-size: 9pt; padding: 3px;")
        image_info_layout.addRow("图像尺寸", self.image_size_label)

        self.frame_rate_label = QLabel("0 fps")
        self.frame_rate_label.setStyleSheet("font-size: 9pt; padding: 3px;")
        image_info_layout.addRow("帧率", self.frame_rate_label)

        self.data_rate_label = QLabel("0 KB/s")
        self.data_rate_label.setStyleSheet("font-size: 9pt; padding: 3px;")
        image_info_layout.addRow("数据率", self.data_rate_label)

        config_layout.addWidget(image_info_group)
        ...
        self.save_image_btn.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        self.save_image_btn.setStyleSheet(get_page_button_style('camera', 'save_image'))
        self.save_image_btn.clicked.connect(self.save_image)
        control_layout.addWidget(self.save_image_btn)

        self.start_capture_btn = QPushButton("▶️开始采集")
        self.start_capture_btn.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        ...
        self.clear_image_btn = QPushButton("🗑清空图像")
        self.clear_image_btn.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        self.clear_image_btn.setStyleSheet(get_page_button_style('camera', 'clear_image'))
        self.clear_image_btn.clicked.connect(self.clear_image)
        control_layout.addWidget(self.clear_image_btn)
        ...
        # 单次扫码按钮
        self.scan_single_btn = QPushButton("📷单次扫码")
        self.scan_single_btn.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        self.scan_single_btn.setStyleSheet(get_page_button_style('camera', 'scan_single', is_3d=True))
        self.scan_single_btn.clicked.connect(self.on_single_scan)
        button_layout.addWidget(self.scan_single_btn)

        # 连续扫码/关闭扫码切换按钮
        self.scan_continuous_btn = QPushButton("🔄连续扫码")
        self.scan_continuous_btn.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        self.scan_continuous_btn.setStyleSheet(get_page_button_style('camera', 'scan_continuous'))
        self.scan_continuous_btn.clicked.connect(self.on_toggle_continuous_scan)
        button_layout.addWidget(self.scan_continuous_btn)

        scan_control_layout.addLayout(button_layout)

        config_layout.addWidget(scan_control_group)
        ...
        """)
        preview_layout.addWidget(self.image_label, 0, Qt.AlignLeft | Qt.AlignTop)  # 靠左上角显示
        ...
        self.scan_result_text.setMaximumHeight(120)
        ...
        """)
        scan_result_layout.addWidget(self.scan_result_text)
        ...
        self.scan_code_type_text = QLineEdit()
        self.scan_code_type_text.setReadOnly(True)
        self.scan_code_type_text.setStyleSheet("""
            QLineEdit {
                border: 1px solid #dcdfe6;
                border-radius: 4px;
                padding: 3px 5px;
        ...
        """)
        code_type_layout.addWidget(self.scan_code_type_text)
        ...
        self.scan_mode_text = QLineEdit()
        self.scan_mode_text.setReadOnly(True)
        self.scan_mode_text.setStyleSheet("""
            QLineEdit {
                border: 1px solid #dcdfe6;
                border-radius: 4px;
                padding: 3px 5px;
        ...
        """)
        scan_mode_layout.addWidget(self.scan_mode_text)
        ...
        self.scan_count_text = QLineEdit()
        self.scan_count_text.setReadOnly(True)
        self.scan_count_text.setStyleSheet("""
            QLineEdit {
                border: 1px solid #dcdfe6;
                border-radius: 4px;
                padding: 3px 5px;
        ...
        """)
        count_layout.addWidget(self.scan_count_text)
        ...
        self.scan_success_rate_text = QLineEdit()
        self.scan_success_rate_text.setReadOnly(True)
        self.scan_success_rate_text.setStyleSheet("""
            QLineEdit {
                border: 1px solid #dcdfe6;
                border-radius: 4px;
                padding: 3px 5px;
        ...
        """)
        rate_layout.addWidget(self.scan_success_rate_text)
        ...
        clear_history_btn.clicked.connect(self.clear_scan_history)
        history_layout.addWidget(clear_history_btn)

        scan_result_layout.addLayout(history_layout)

        # 扫码历史文本框
        self.scan_history_text = QTextEdit()
        self.scan_history_text.setReadOnly(True)
        self.scan_history_text.setMaximumHeight(240)
        self.scan_history_text.setStyleSheet("""
            QTextEdit {
                border: 1px solid #dcdfe6;
                border-radius: 4px;
                padding: 5px;
        ...
        """)
        scan_result_layout.addWidget(self.scan_history_text)
        ...
        preview_splitter.addWidget(scan_result_group)
        ...
        self.auto_scroll_log_check = QCheckBox("自动滚动")
        self.auto_scroll_log_check.setChecked(True)
        self.auto_scroll_log_check.setStyleSheet("font-size: 9pt;")
        log_options_layout.addWidget(self.auto_scroll_log_check)
        ...
        self.hex_display_check.setStyleSheet("font-size: 9pt;")
        options_layout.addWidget(self.hex_display_check)

        self.auto_scroll_check = QCheckBox("自动滚动")
        self.auto_scroll_check.setChecked(True)
        self.auto_scroll_check.setStyleSheet("font-size: 9pt;")
        options_layout.addWidget(self.auto_scroll_check)

        self.timestamp_check = QCheckBox("显示时间戳")
        self.timestamp_check.setChecked(False)
        self.timestamp_check.setStyleSheet("font-size: 9pt;")
        ...
        self.data_text.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
    ...
    def process_yuv422(self, data):
        ...
        try:
            ...
            # 记录接收到的数据长度和预期长度
            if self.image_type == "灰度图像":
                expected_size = self.image_width * self.image_height
            else:
                expected_size = self.image_width * self.image_height * 2
            Logger.debug(f"处理YUV422数据: 接收长度={len(data_array)}, 预期长度={expected_size}, 图像尺寸={self.image_width}x{self.image_height}", module='camera')
            ...
            if self.image_type == "灰度图像":
                ...
                try:
                    # 将数据重塑为图像尺寸
                    gray = data_array.reshape((self.image_height, self.image_width))
                ...
                # 彩色图像，处理完整的YUV422数据
                # 根据YUV格式转换
                yuv_format = self.yuv_format_combo.currentText()
                ...
                # 重塑YUV分量
                try:
                    y = y.reshape((self.image_height, self.image_width))
                    u = u.reshape((self.image_height, self.image_width // 2))
                    v = v.reshape((self.image_height, self.image_width // 2))
    ...
    def process_yuv420(self, data):
        ...
        try:
            ...
            # 检查数据长度是否足够
            expected_size = self.image_width * self.image_height * 3 // 2
            if len(data_array) < expected_size:
                return
            ...
            y = data_array[:y_size].reshape((self.image_height, self.image_width))
            u = data_array[y_size:y_size + y_size // 4].reshape((self.image_height // 2, self.image_width // 2))
            v = data_array[y_size + y_size // 4:].reshape((self.image_height // 2, self.image_width // 2))

            # 上采样U和V分量
            u = cv2.resize(u, (self.image_width, self.image_height))
            v = cv2.resize(v, (self.image_width, self.image_height))
    ...
    def process_rgb565(self, data):
        ...
        try:
            ...
            # 检查字节顺序
            if self.byte_order_combo.currentText() == "Big Endian":
    ...
    def process_rgb888(self, data):
        ...
        try:
            ...
            # 重塑为RGB图像
            rgb = data_array.reshape((self.image_height, self.image_width, 3))
    ...
    def process_scan_data(self, data: bytes):
        ...
        try:
            ...
            Logger.debug(f"开始处理扫码数据,缓冲区大小={len(self.image_buffer)}字节", module='camera')
            ...
            while len(self.image_buffer) >= SCAN_HEADER_SIZE:
                ...
                if len(self.image_buffer) < total_scan_size:
                    # 数据不足,等待更多数据
                    Logger.debug(f"数据不足,当前={len(self.image_buffer)}字节,需要={total_scan_size}字节,等待更多数据", module='camera')
                    return  # 修改: 直接返回而不是break
                ...
                # 更新扫码结果显示
                self.update_scan_result(result_data, type_name, success)

                # 从缓冲区移除已处理的扫码数据
                Logger.debug(f"从缓冲区移除已处理的{total_scan_size}字节扫码数据", module='camera')
                self.image_buffer = self.image_buffer[total_scan_size:]

            Logger.debug(f"扫码数据处理完成,剩余缓冲区大小={len(self.image_buffer)}字节", module='camera')
